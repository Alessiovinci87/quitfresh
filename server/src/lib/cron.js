...
function startCron() {
  ...
  cron.schedule('* * * * *', async () => {
    ...
    try {
      ...
      // Pre-calcola start/end di oggi in Europe/Rome per query pillsTaken.
      ...
      for (const user of cytisineUsers) {
        ...
        //     segnato la capsula in Home (pillsTaken < expected), re-invia.
        //     Cap a 60 min: oltre, l'utente ha probabilmente saltato.
        ...
        if (actualTaken >= expectedTaken) continue; // confermato, no reminder
